...
export default function ShoppingList({
  ...
  return (
    <div className="space-y-4">
      ...
      {showAddForm && (
        <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl p-3 flex gap-2 items-end">
          <div className="flex-1">
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Nazwa"
              className="w-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg px-3 py-1.5 text-white text-sm" />
          </div>
          <div className="w-20">
          ...
          <select value={unit} onChange={e => setUnit(e.target.value)}
            className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg px-2 py-1.5 text-white text-sm">
            <option value="g">g</option><option value="kg">kg</option>
            <option value="ml">ml</option><option value="l">l</option>
            <option value="szt">szt</option>
          </select>
          <button onClick={handleAdd} className="px-3 py-1.5 bg-violet-600 text-white text-sm rounded-lg">OK</button>
        </div>
      )}
      ...
      {toBuy.length > 0 && (
        <div className="space-y-1">
          {toBuy.map(item => (
            <div key={item.id} className="flex items-center gap-3 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg p-3">
              <button onClick={() => onToggleBought(item.id)}
                className="w-5 h-5 rounded border-2 border-[var(--card-border)] hover:border-emerald-500 transition-colors" />
              <span className="text-white flex-1">{item.name}</span>
              <span className="text-[var(--muted)] text-sm">{item.amount} {item.unit}</span>
              <button onClick={() => onDelete(item.id)} className="text-[var(--muted)] hover:text-red-400">
      ...
      {bought.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-[var(--muted)]">Kupione ({bought.length})</p>
            <button onClick={onClearBought} className="text-xs text-[var(--muted)] hover:text-red-400">Wyczyść</button>
          </div>
          <div className="space-y-1 opacity-60">
            {bought.map(item => (
              <div key={item.id} className="flex items-center gap-3 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg p-3">
                ...
                <span className="text-[var(--muted)] flex-1 line-through">{item.name}</span>
                <span className="text-[var(--muted)] text-sm">{item.amount} {item.unit}</span>
              </div>
            ))}
          </div>

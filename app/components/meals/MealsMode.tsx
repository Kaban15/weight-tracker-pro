...
import { useMealEnrichment } from './useMealEnrichment';
import { useMealTrackerSync } from './useMealTrackerSync';
...
export default function MealsMode({ onBack }: MealsModeProps) {
  ...
  const { enrichIngredientsWithNutrition } = useMealEnrichment({
    userId: user?.id,
    mealPlans,
    isLoading,
    lookupNutrition,
  ...
  const {
    trackerMealKeys,
    toast: trackerToast,
    clearToast: clearTrackerToast,
    autoSyncToTracker,
    handleSendToTracker,
  } = useMealTrackerSync({ userId: user?.id, mealPlansLength: mealPlans.length });
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] p-4">
      <div className="max-w-4xl mx-auto">
        ...
        {resolvedView === 'pantry' && (
          <PantryManager
            ...
            writeOffs={{
              items: pantryWriteOffs.writeOffs,
              monthlyTotal: pantryWriteOffs.monthlyTotal,
              monthlyCount: pantryWriteOffs.monthlyCount,
              loading: pantryWriteOffs.loading,
              onWriteOff: handleWriteOff,
              onDelete: handleDeleteWriteOff,
              onLoad: pantryWriteOffs.loadWriteOffs,
            }}
      ...
      {(toast || trackerToast) && <Toast message={(toast || trackerToast)!.message} type={(toast || trackerToast)!.type} onClose={() => { setToast(null); clearTrackerToast(); }} />}
    </div>
  );
}

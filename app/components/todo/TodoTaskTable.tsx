  ...
  if (tasks.length === 0) {
    return (
      <div className="bg-[var(--card-bg)] rounded-xl p-12 border border-[var(--card-border)] text-center">
        <ListTodo className="w-16 h-16 text-[var(--muted)] mx-auto mb-4" />
        ...
        {hasActiveFilters ? (
          <button
            ...
            className="bg-[var(--surface)] hover:bg-[var(--surface)] text-white px-6 py-3 rounded-xl font-semibold inline-flex items-center gap-2"
          >
            <X className="w-5 h-5" />
            Wyczyść filtry
          </button>
          ...
          <button
            onClick={onAddTask}
            className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-semibold inline-flex items-center gap-2"
          >
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[var(--surface)]">
              <th className="px-4 py-3 text-left text-sm font-semibold text-[var(--foreground)] w-10" />
              <th className="px-4 py-3 text-left text-sm font-semibold text-[var(--foreground)]">
                Zadanie
              </th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-[var(--foreground)] w-32">
                Termin
              </th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-[var(--foreground)] w-36">
                Priorytet
              </th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-[var(--foreground)] w-40">
                Status
              </th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-[var(--foreground)] w-36">
                Kategoria
              </th>
              <th className="px-4 py-3 text-right text-sm font-semibold text-[var(--foreground)] w-20">
                Akcje
              </th>
            </tr>
          </thead>
          <tbody>
            {tasks.map((task) => {
              ...
              return (
                <tr
                  key={task.id}
                  className={`border-t border-[var(--card-border)] hover:bg-[var(--surface)] transition-colors ${
                    task.completed || isCancelled ? "opacity-60" : ""
                  } ${overdueTask ? "bg-red-900/20" : ""} ${
                ...
                >
                    ...
                    <button
                      ...
                      className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-colors ${
                        task.completed
                          ...
                          : isCancelled
                          ? "bg-[var(--surface)] border-[var(--card-border)] cursor-not-allowed"
                          : "border-[var(--card-border)] hover:border-[var(--accent)]"
                      }`}
                    >
                      {task.completed && <Check className="w-4 h-4 text-[var(--foreground)]" />}
                      {isCancelled && <X className="w-4 h-4 text-[var(--muted)]" />}
                    </button>
                  </td>
                    ...
                    <div className="flex flex-col">
                      <span
                        className={`text-white ${
                          task.completed || isCancelled ? "line-through text-[var(--muted)]" : ""
                        }`}
                      >
                        {task.title}
                      </span>
                      {task.notes && (
                        <span className="text-[var(--muted)] text-xs mt-1 flex items-center gap-1">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-sm ${
                        overdueTask
                          ? "text-blue-400 font-semibold bg-blue-500/20 px-2 py-1 rounded"
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        className="p-1.5 text-[var(--muted)] hover:text-blue-400 hover:bg-[var(--surface)] rounded transition-colors"
                      >
